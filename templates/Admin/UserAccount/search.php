...
use App\Infrastructure\Persistence\Cake\User\UserAccountsRepository\Mapper as UserAccountMapper;
        ...
        <table class="table table-hover table-bordered mb-0">
            ...
            <?php } else { ?>
                <?php foreach (collection($rows)
                    ->map(fn(OrmEntity $e) => (new UserAccountMapper)->toDomainEntity($e))
                    ->toList() as $row): ?>
                <tr>
                    <td><?= h($row->id()) ?></td>
                    <td><?= h($row->email()) ?></td>
                    ...
                    <td class="text-nowrap">
                        <a href="<?= $this->Url->build([
                            ...
                            'action' => 'index',
                            'account_id' => $this->getRequest()->getParam('account_id'),
                            'user_account_id' => $row->id()->toString(),
                            '?' => $this->getRequest()->getQuery(),
