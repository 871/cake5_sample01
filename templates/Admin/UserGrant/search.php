...
?>
<div class="card mb-3">
    <div class="card-header bg-secondary text-white">ユーザ権限検索</div>
    <div class="card-body">
        <form method="get" class="row g-3">
            <div class="col-md-2">
                <label class="form-label">ユーザID</label>
                <input type="number" min="1" name="user_account_id" class="form-control" value="<?= h($this->getRequest()->getQuery('user_account_id')) ?>">
            </div>
            <div class="col-md-3">
                <label class="form-label">ユーザステータス</label>
                <select 
    ...
    <div class="card-body p-0">
        <table class="table table-bordered table-hover mb-0">
            <thead class="table-dark">
                <tr>
                    <th>
                        <?= $this->Paginator->sort('UserAccounts.id', 'ユーザID', $pageOptions) ?>
                        <br>
                        ステータス
                    </th>
                    <th>
                        <?= $this->Paginator->sort('UserAccounts.name', 'ユーザ名', $pageOptions) ?>
                        <br>
                        <?= $this->Paginator->sort('UserAccounts.email', 'メール', $pageOptions) ?>
                    </th>
                    <th>
                        権限名
                        <br>
